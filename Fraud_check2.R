...
#Model Buildingr
modelA<-C5.0(Fraud_train[,-c(7)],Fraud_train$taxableinc)
summary(modelA)
plot(modelA)

# Training accuracy
pred_train <- predict(modelA,Fraud_train)
mean(Fraud_train$taxableincome==pred_train) 
library(caret)
confusionMatrix(pred_train,Fraud_train$taxableinc)

pred_test <- predict(modelA,newdata=fraud_test) # predicting on test data
